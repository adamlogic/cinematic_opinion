...
(function() {
  var svg, svgChart, xScale, yScale;

  xScale = d3.scale.linear().range([0, Chart.width]).domain([0, 100]);

  yScale = d3.scale.linear().range([Chart.height, 0]).domain([0, 100]);

  svg = d3.select('#chart').attr('width', Chart.width + Chart.margin.left).attr('height', Chart.height + Chart.margin.bottom);

  Chart.addChartBackground(svg);

  svgChart = svg.append('g').attr('transform', "translate(" + Chart.margin.left + ",0)");

  d3.tsv('data.tsv', function(error, data) {
    data.forEach(function(d) {
      d.tomatoScore = +d.tomatoScore;
      return d.audienceScore = +d.audienceScore;
    });
    data = data.filter(function(d) {
      return d.tomatoScore > 0 && d.audienceScore > 0;
    });
    Chart.addDots(svgChart, data, xScale, yScale);
    return Chart.addYearFilter(data, function(data) {
      Chart.removeDots(svgChart, data);
      return Chart.addDots(svgChart, data, xScale, yScale);
    });
  });
